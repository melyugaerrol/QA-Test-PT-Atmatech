# Generated by Selenium IDE
import pytest
import time
import json
from selenium import webdriver
from selenium.webdriver.common.by import By
from selenium.webdriver.common.action_chains import ActionChains
from selenium.webdriver.support import expected_conditions
from selenium.webdriver.support.wait import WebDriverWait
from selenium.webdriver.common.keys import Keys
from selenium.webdriver.common.desired_capabilities import DesiredCapabilities

class Test2oldest():
  def setup_method(self, method):
    self.driver = webdriver.Chrome()
    self.vars = {}
  
  def teardown_method(self, method):
    self.driver.quit()
  
  def test_2oldest(self):
    # Test name: 2_oldest
    # Step # | name | target | value
    # 1 | open | / | 
    self.driver.get("https://brand.netray.id/")
    # 2 | setWindowSize | 1050x708 | 
    self.driver.set_window_size(1050, 708)
    # 3 | click | css=.list-inline | 
    self.driver.find_element(By.CSS_SELECTOR, ".list-inline").click()
    # 4 | runScript | window.scrollTo(0,52) | 
    self.driver.execute_script("window.scrollTo(0,52)")
    # 5 | runScript | window.scrollTo(0,1267) | 
    self.driver.execute_script("window.scrollTo(0,1267)")
    # 6 | click | css=.section:nth-child(4) .list-group-item:nth-child(2) .text-truncate | 
    self.driver.find_element(By.CSS_SELECTOR, ".section:nth-child(4) .list-group-item:nth-child(2) .text-truncate").click()
    # 7 | click | css=div:nth-child(2) > div > .d-flex > .custom-select | 
    self.driver.find_element(By.CSS_SELECTOR, "div:nth-child(2) > div > .d-flex > .custom-select").click()
    # 8 | select | css=div:nth-child(2) > div > .d-flex > .custom-select | label=Oldest
    dropdown = self.driver.find_element(By.CSS_SELECTOR, "div:nth-child(2) > div > .d-flex > .custom-select")
    dropdown.find_element(By.XPATH, "//option[. = 'Oldest']").click()
    # 9 | mouseOver | css=div:nth-child(2) > div > .row #handleMarkAllSelect | 
    element = self.driver.find_element(By.CSS_SELECTOR, "div:nth-child(2) > div > .row #handleMarkAllSelect")
    actions = ActionChains(self.driver)
    actions.move_to_element(element).perform()
    # 10 | mouseOut | css=div:nth-child(2) > div > .row #handleMarkAllSelect | 
    element = self.driver.find_element(By.CSS_SELECTOR, "body")
    actions = ActionChains(self.driver)
    actions.move_to_element(element, 0, 0).perform()
    # 11 | click | css=.p-2 | 
    self.driver.find_element(By.CSS_SELECTOR, ".p-2").click()
    # 12 | close |  | 
    self.driver.close()
  
